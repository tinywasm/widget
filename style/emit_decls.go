...
func (r rule) emitsNothing(layer widget.Layer) bool {
	...
	return !r.hasFlow && !r.fill && !r.grow && !r.pushEnd && !r.scroll && !r.keepSize && !r.edgeToEdge && !r.hideOverflow && !r.hasIcon && !r.controlBox && !r.logoBox && !r.chipBox && !r.hasGlyph && !r.hasPadEdge && !r.hasChipSeat && !r.hasPadInline && !r.startContent && !r.shown && !r.hasRotate && !r.hasCapped && !r.hasDivider && !r.hasDividerBelow && !r.hasDividerBetween && !r.hasFloatMiddle
}
